...
=============================================================
  ...
  Cambios:
    - YOLO detecta todas las clases, etiqueta siempre "TROZO DE COMIDA"
    - Depth colormap: TURBO (arcoíris, máxima diferenciación)
...
# COLORMAP para depth — TURBO da el mayor contraste visual:
#   azul (lejos) → verde → amarillo → rojo (cerca)
#   Alternativas: cv2.COLORMAP_JET, cv2.COLORMAP_HSV, cv2.COLORMAP_RAINBOW
...
# 1. YOLO-World — todas las clases originales para máxima detección
#    La etiqueta visible siempre será "TROZO DE COMIDA" (ver más abajo)
...
def run_depth_async(frame_rgb):
    ...
    # Normalizar con clip para aumentar contraste en la zona de interés
    # (descartamos el 2% más extremo de cada lado para no saturar)
    ...
    # TURBO: azul = lejos, rojo = cerca — máxima diferenciación de color
...
print("   Depth colormap: TURBO  (azul=lejos → verde → amarillo → rojo=cerca)\n")

# ──────────────────────────────────────────────
#  LEYENDA DE PROFUNDIDAD (barra lateral)
# ──────────────────────────────────────────────
def dibujar_leyenda_depth(frame_in):
    """Barra vertical TURBO con etiquetas CERCA / LEJOS."""
    h, w = frame_in.shape[:2]
    bar_h  = 200
    bar_w  = 20
    margin = 15
    x0 = w - bar_w - margin - 50
    y0 = h // 2 - bar_h // 2

    # Gradiente vertical
    gradient = np.linspace(255, 0, bar_h, dtype=np.uint8).reshape(bar_h, 1)
    gradient = cv2.applyColorMap(gradient, DEPTH_COLORMAP)          # (bar_h,1,3)
    gradient = cv2.resize(gradient, (bar_w, bar_h))

    frame_in[y0:y0+bar_h, x0:x0+bar_w] = gradient
    cv2.rectangle(frame_in, (x0, y0), (x0+bar_w, y0+bar_h), (200,200,200), 1)

    font  = cv2.FONT_HERSHEY_SIMPLEX
    scale = 0.42
    cv2.putText(frame_in, "CERCA", (x0 - 2, y0 - 6),   font, scale, (100,100,255), 1)
    cv2.putText(frame_in, "LEJOS", (x0 - 2, y0+bar_h+14), font, scale, (255,100,100), 1)
    return frame_in
...
while True:
    ...
    if show_yolo:
        ...
        for box in results.boxes:
            ...
            cls = int(box.cls[0])
            alimentos.append({
                'clase':  "TROZO DE COMIDA",  # siempre esta etiqueta, sin importar qué detectó
    ...
    # Overlay depth — peso mayor (0.45) para que se vea bien el colormap TURBO
    if show_depth and d_vis is not None:
        frame = cv2.addWeighted(frame, 0.55, cv2.resize(d_vis, (w, h)), 0.45, 0)
        frame = dibujar_leyenda_depth(frame)
    ...
    if alimentos:
        ...
        print(f"\r🎯 {alimentos[0]['clase']:<20} "
    ...
    cv2.putText(frame,
                f"YOLO: {'ON' if show_yolo else 'OFF'} ({len(alimentos)} trozos)",
                (10,50), cv2.FONT_HERSHEY_SIMPLEX, 0.55,
                (0,255,100) if show_yolo else (100,100,100), 1)
    cv2.putText(frame,
                f"Depth TURBO: {'ON' if show_depth else 'OFF'}"
    ...
    cv2.imshow("Brazo Robotico | YOLO-World + Depth TURBO", frame)
