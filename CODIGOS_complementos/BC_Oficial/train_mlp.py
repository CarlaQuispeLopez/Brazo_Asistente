"""
train_mlp.py  —  Entrena el MLP de Behavior Cloning
=====================================================
Input:  (cx_norm, cy_norm)          — posición normalizada del trozo en la imagen
Output: (base_pasos, codo_pasos, hombro_pasos)  — pasos netos por articulación

Carga demonstrations_clean.pkl, entrena la red y guarda:
  - modelo_bc.pt   →  pesos de la red + parámetros de normalización

Uso:
  python train_mlp.py
  python train_mlp.py --epochs 3000 --lr 0.0005
"""
...
#  HIPERPARÁMETROS
# ─────────────────────────────────────────────
HIDDEN       = [128, 128, 64]   # capas ocultas
DROPOUT      = 0.05
LR           = 0.001
EPOCHS       = 3000
BATCH_SIZE   = 32
...
class BrazoMLP(nn.Module):
    """
    Red neuronal pequeña que aprende la función
        (cx_norm, cy_norm) → (Δbase, Δcodo, Δhombro)
    en espacio normalizado.
    """
...
def cargar_datos(pkl_path):
    """
    Lee demonstrations_clean.pkl y extrae pares (cx, cy) → (B, C, H).
    Aplica las mismas reglas de limpieza que limpiar_dataset_complete.py:
      - Solo episodios con celda_objetivo != None
      - Elimina success=False excepto celda=16
      - Suma pasos por eje (ya viene limpio en el pkl)
    """
    ...
    for ep in eps:
        ...
        # Validar que tiene posición y celda
        ...
        # Extraer pasos netos de los steps limpios
        b = c = h = 0
...
def normalizar(Y_train):
    """StandardScaler manual → devuelve (mean, std) para desnormalizar."""
...
def entrenar(X, Y, epochs=EPOCHS, lr=LR, batch_size=BATCH_SIZE,
    ...
    criterion = nn.HuberLoss(delta=1.0)   # robusto a outliers
    ...
    for ep in range(1, epochs + 1):
        ...
        with torch.no_grad():
            ...
            # RMSE en pasos reales (desnormalizado)
...
def guardar_modelo(model, y_mean, y_std, output_path):
    """
    Guarda en un solo .pt:
      - Los pesos de la red
      - Los parámetros de normalización (mean, std)
      - La arquitectura (hidden layers)
    """
    checkpoint = {
