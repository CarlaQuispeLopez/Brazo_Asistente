...
EATING_CM    =  8.0   # Distancia "persona comiendo" (muy cerca)
FAR_CM       = 25.0   # Distancia "persona alejada" (terminó de comer)
...
def captura_automatica(cuadricula, yolo_model, simulate=False):
    ...
    mejor = None
    print(f"  Buscando trozo objetivo (máx. {DETECT_FRAMES} frames)...")

    for frame_idx in range(DETECT_FRAMES):
        ret, frame = cap.read()
        if not ret:
            print(f"  WARN: frame {frame_idx} no disponible.")
            cv2.waitKey(30)
            continue
        ...
        estado = f"Frame {frame_idx+1}/{DETECT_FRAMES} | Trozos en malla: {len(candidatos)}"
        cv2.putText(preview, estado, (10, 32),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,255,255), 2)
        cv2.putText(preview, "Deteccion AUTOMATICA — sin teclas requeridas",
                    (10, 62), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (200,200,200), 1)
...
def fase_entrega(robot, simulate=False,
    ...
    if not cap2.isOpened():
        print(f"  ✗ ERROR: No se pudo abrir cámara {CAMERA_2_INDEX}. Saltando entrega.")
        return
    ...
    ventana2 = f"MediaPipe Seguridad — Cámara {CAMERA_2_INDEX}"
    ...
    while approach_idx < MAX_APPROACH_MOVES:
        ...
        # Zona segura: ejecutar movimiento de aproximación
        eje, pasos = APPROACH_SEQ[approach_idx % len(APPROACH_SEQ)]
        if dist is not None:
            print(f"  dist={dist:.1f}cm — moviendo {eje} +{pasos} (paso {approach_idx+1})")
        else:
            print(f"  Sin rostro detectado — moviendo {eje} +{pasos} (paso {approach_idx+1})")
...
def main(simulate=False, threshold_cm=THRESHOLD_CM,
    ...
    with RobotInterface(simulate=simulate) as robot:
        ...
        while continuar:
            ...
            if objetivo is None:
                print("\n  ✗ Plato vacío — no hay más trozos. FIN del programa.")
                continuar = False
                break
