...
class Cuadricula:
    ...
    # ── Métodos de ESQUINAS (intersecciones) ────────────────────────────────

    def num_esquinas(self):
        """Total de esquinas: (cols+1) * (rows+1)."""
        return (self.cols + 1) * (self.rows + 1)

    def esquina_num(self, col_e, row_e):
        """Número de esquina (1-based) a partir de col/row de intersección."""
        return row_e * (self.cols + 1) + col_e + 1

    def esquina_colrow(self, num):
        """Devuelve (col_e, row_e) de la esquina numerada (1-based)."""
        idx   = num - 1
    ...
    def esquina_px(self, num):
        """Posición en píxeles del centro de la esquina num."""
        col_e, row_e = self.esquina_colrow(num)
        return self._t([[col_e, row_e]])[0]

    def todas_esquinas_px(self):
        """Lista de (num, px_array) para todas las esquinas."""
        result = []
    ...
    def esquina_mas_cercana(self, px, py):
        """
        Dado un punto en píxeles, devuelve la esquina más cercana.
        Retorna dict: {esquina, col_e, row_e, px_img, py_img, cx_norm, cy_norm}
        """
        if not self.disponible:
            return None
        # Transformar a espacio de cuadrícula
        gx, gy = cv2.perspectiveTransform(
            np.float32([[[px, py]]]), self.M_inv)[0][0]
        # Esquina más cercana en espacio de cuadrícula (redondeo)
        col_e = int(round(np.clip(gx, 0, self.cols)))
        row_e = int(round(np.clip(gy, 0, self.rows)))
        num   = self.esquina_num(col_e, row_e)
        # Posición en píxeles de esa esquina
    ...
    def dibujar(self, frame, highlight=None, celdas_usadas=None,
        ...
        """
        highlight        : celda actual del objetivo (naranja)
        celdas_usadas    : dict celda->count  (se pinta rojo)
        esquina_highlight: número de esquina seleccionada (círculo cyan)
        esquinas_usadas  : dict esquina->count (círculos rojos rellenos)
        """
        if not self.disponible:
            return frame
        ...
        # ── relleno semitransparente de celdas ───────────────────────────
        overlay = frame.copy()
        ...
        # resalte sólido celdas con demos
        ...
        # ── CÍRCULOS EN LAS ESQUINAS (intersecciones) ────────────────────
        RADIO        = 12
        COLOR_NORMAL = (0, 0, 220)      # rojo oscuro vacío
        COLOR_USADO  = (0, 0, 255)      # rojo brillante relleno
        COLOR_TARGET = (0, 255, 255)    # cian: esquina objetivo actual
        ...
        for row_e in range(self.rows + 1):
            for col_e in range(self.cols + 1):
                ...
                if num_e == esquina_highlight:
                    # Esquina objetivo actual: cian relleno + borde blanco
                    ...
                    # Esquina con demos: rojo relleno + número de demos
                    cv2.circle(frame, centro, RADIO, COLOR_USADO, -1)
                    cv2.circle(frame, centro, RADIO, (255, 255, 255), 1)
                    # Pequeño contador de demos
                    txt_e = str(cnt_e)
                    ...
                    # Esquina sin demo: círculo vacío rojo
                    cv2.circle(frame, centro, RADIO, COLOR_NORMAL, 2)
                    # Cruz interna amarilla
                    cv2.line(frame,
...
class FoodDetection:
    def __init__(self, label, confidence, center_norm, center_px,
        ...
        self.esquina_info = esquina_info   # esquina más cercana al trozo
...
class VisionPipeline:
    ...
    def get_detections(self, frame, celdas_usadas=None, esquinas_usadas=None):
        """Devuelve (lista_detecciones, frame_anotado_con_malla_circulos_yolo)."""
        if celdas_usadas   is None: celdas_usadas   = {}
        if esquinas_usadas is None: esquinas_usadas  = {}
        ...
        # malla + círculos en esquinas, luego YOLO encima
...
def capturar_alimento(vision, celdas_usadas=None, esquinas_usadas=None):
    """
    Abre ventana con YOLO + malla + círculos en esquinas.
    - Esquinas con demos → círculo ROJO relleno
    - Esquina más cercana al objetivo → círculo CYAN
    ESPACIO → confirma la ESQUINA más cercana al trozo detectado.
    ESC     → cancelar.
    ...
    while True:
        ...
        if key == 32:
            ...
            # Posición normalizada de la ESQUINA (no del centro del bbox)
            ...
            resultado = FoodDetection(
                "Trozo_Comida",
                best["conf"],
                (cx_norm_esq, cy_norm_esq),          # ← pos de la esquina
...
class DemoCollector:
    ...
    def _esquinas_usadas(self):
        """Devuelve dict {esquina: cantidad_demos} de los episodios guardados."""
        from collections import Counter
        eps     = self.data.get("episodes", [])
    ...
    def cmd_cam(self):
        """Abre ventana con YOLO + malla + círculos. ESPACIO confirma la esquina."""
