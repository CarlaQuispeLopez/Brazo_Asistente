...
DEMO_FILE       = r"C:\Users\MSI LAPTOP\Documents\BRAZO2\CODIGOS_NUEVO\demos\demonstrations.pkl"
CHECKPOINT_FILE = r"C:\Users\MSI LAPTOP\Documents\BRAZO2\CODIGOS_NUEVO\demos\checkpoint_ep_activo.pkl"
...
class Cuadricula:
    ...
    def dibujar(self, frame, highlight=None, celdas_usadas=None):
        """
        highlight      : celda actual del objetivo (azul oscuro)
        celdas_usadas  : set/dict de celda -> cantidad de demos (se pinta rojo)
        """
        if not self.disponible:
            return frame

        if celdas_usadas is None:
            celdas_usadas = {}
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                if num == highlight:
                    color = (0, 0, 100)        # azul oscuro: objetivo actual
                elif num in celdas_usadas:
                    color = (0, 0, 180)        # rojo oscuro: ya tiene demos
                else:
                    color = self.COLOR_RELLENO
                cv2.fillPoly(overlay, [pts], color)
        cv2.addWeighted(overlay, 0.35, frame, 0.65, 0, frame)

        # resalte sólido de celda objetivo
        if highlight and 1 <= highlight <= self.cols * self.rows:
            ...
            cv2.fillPoly(frame, [pts], (180, 60, 0))   # azul brillante

        # resalte sólido de celdas con demos
        for celda_num in celdas_usadas:
            ...
            pts = self._t([[c,r],[c+1,r],[c+1,r+1],[c,r+1]]).astype(np.int32)
            cv2.fillPoly(frame, [pts], (0, 0, 200))    # rojo brillante
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                # texto: número + cantidad de demos si tiene
                ...
                if num == highlight:
                    ...
                    color = (255, 255, 255)    # blanco sobre rojo
                else:
...
class RobotInterface:
    ...
    def __enter__(self):
        if not self.simulate:
            ...
            deadline = time.time() + 20   # Arduino mueve a HOME al arrancar
...
# ─────────────────────────────────────────────
class FoodDetection:
    def __init__(self, label, confidence, center_norm, center_px, celda_info):
        self.label       = label
...
class VisionPipeline:
    ...
    def get_detections(self, frame, celdas_usadas=None):
        """Devuelve (lista_detecciones, frame_anotado_con_malla_y_yolo)."""
        if celdas_usadas is None:
            celdas_usadas = {}
        ...
        # malla primero (con celdas_usadas en rojo), luego YOLO encima
        ...
        cv2.putText(annotated,
                    f"Trozos: {n}   |   ESPACIO = confirmar objetivo   ESC = cancelar   "
                    f"Rojo = celda con demos ({len(celdas_usadas)} celdas usadas)",
...
# ─────────────────────────────────────────────
#  CAPTURA INTERACTIVA CON ESPACIO
# ─────────────────────────────────────────────
def capturar_alimento(vision, celdas_usadas=None):
    """
    Abre ventana con YOLO + malla en vivo.
    - Celdas con demos ya grabadas → ROJO
    - Celda del objetivo actual    → naranja/azul
    ESPACIO → confirmar.   ESC → cancelar.
    Devuelve FoodDetection o None.
    """
    if celdas_usadas is None:
        celdas_usadas = {}

    VENTANA = "CAPTURA  |  ESPACIO = confirmar   ESC = cancelar"

    print("\n  Abriendo ventana de captura...")
    print("  Apunta al plato y pulsa  ESPACIO  para confirmar el objetivo.")
    print(f"  Celdas con demos (en ROJO): {sorted(celdas_usadas.keys())}")
    print("  ESC para cerrar sin confirmar.\n")

    # ── Crear ventana y forzar que aparezca al frente ─────────────────────
    ...
    # ── Mostrar pantalla negra inmediatamente para que la ventana aparezca
    pantalla_inicio = np.zeros((650, 1100, 3), dtype=np.uint8)
    cv2.putText(pantalla_inicio, "Iniciando camara...",
                (300, 320), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 255), 2)
    cv2.putText(pantalla_inicio, "ESPACIO = confirmar objetivo   ESC = cancelar",
                (200, 380), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 1)
    cv2.imshow(VENTANA, pantalla_inicio)
    cv2.waitKey(1)   # necesario para que Windows procese la ventana

    # ── warm-up: leer frames hasta tener imagen real ──────────────────────
    for _ in range(15):
        f = vision.read_frame()
        if f is not None:
    ...
    while True:
        ...
        detections, annotated = vision.get_detections(frame,
                                                       celdas_usadas=celdas_usadas)
        ...
        if key == 32:          # ESPACIO
            ...
            resultado = FoodDetection(
                ...
                (int(best["cx_px"]), int(best["cy_px"])),
                ci,
            )
            ya_tiene = celdas_usadas.get(ci["celda"], 0)
            print(f"  Objetivo confirmado:")
            print(f"    Confianza   : {best['conf']:.2f}")
            print(f"    Centro px   : {resultado.center_px}")
            print(f"    Centro norm : cx={best['cx_norm']:.4f}  cy={best['cy_norm']:.4f}")
            if ci["celda"]:
                aviso = f"  *** ya tiene {ya_tiene} demo(s) ***" if ya_tiene else ""
                print(f"    Celda malla : {ci['celda']}  "
                      f"(Fila {ci['fila']}, Col {ci['columna']}){aviso}")
            else:
                print(f"    Celda malla : fuera de la malla")
            print()
            break

    cv2.destroyWindow(VENTANA)
    cv2.waitKey(1)   # procesar el cierre
    return resultado
...
class DemoCollector:
    ...
    def _celdas_usadas(self):
        """Devuelve dict {celda: cantidad_demos} de los episodios guardados."""
        from collections import Counter
    ...
    def cmd_cam(self):
        """Abre ventana con YOLO + malla. ESPACIO confirma el objetivo."""
        print("  Cerrando garra antes de capturar...")
        self.robot.set_gripper_angle(PINZA_MIN)   # cerrar garra al usar CAM
        food = capturar_alimento(self.vision, celdas_usadas=self._celdas_usadas())
        if food is None:
    ...
    def cmd_iniciar(self):
        ...
        if self._food_target:
            ...
            print(f"  Objetivo en celda: {ci.get('celda','?')}  "
                  f"(Fila {ci.get('fila','?')}, Col {ci.get('columna','?')})")
    ...
    def cmd_finalizar(self):
        ...
        print(f"Episodio {self._ep_count()} guardado. "
              f"Celda={ci['celda']}  Pasos={n}  Exito={success}")

        self.robot.go_home()
        print("  Abriendo garra...")
        self.robot.set_gripper_angle(PINZA_MAX)   # abrir garra al llegar a HOME
        self._reset()
    ...
    def run(self):
        ...
        print("  CAM          — abrir camara (YOLO+malla), ESPACIO para confirmar objetivo")
        ...
        while True:
            ...
            ft  = self._food_target
            cel = (f"C{ft.celda_info['celda']}"
                   if (ft and ft.celda_info["celda"]) else "sin_obj")
            try:
